...
public class NameEditing<T extends SavedPosition> {
    ...
    public NameEditing(@NotNull T position, GuiElement.Click click, @NotNull ListMenu<T> parentMenu, int pageNumber , HuskHomesGui plugin){
    ...
    public void open(Player player){
        ...
        Dialog dialog = Dialog.create(builder ->
                builder.empty()
                        .base(DialogBase.builder(localized("dialog_edit_name_title"))
                                .body(List.of(
                                        DialogBody.plainMessage(localized("dialog_edit_name_body"))
                                ))

                                .inputs(List.of(
                                        DialogInput.text("huskhomesgui_home_name", localized("dialog_edit_name_input"))
                                                .initial("")
                                                .width(300)
                                                .build()
                                ))
                        ...
                        .type(DialogType.confirmation(
                                ActionButton.create(
                                        ...
                                        DialogAction.customClick((view, audience) -> {
                                                    ...
                                                    String newName = view.getText("huskhomesgui_home_name"); // 可能為 null
                                                    if (newName == null || newName.isEmpty() ) return;

                                                    try {
                                                        if (position instanceof Home home) {
                                                            plugin.getHuskHomes().getManager().homes().setHomeName(home, newName);
                                                        } else if (position instanceof Warp warp) {
                                                            plugin.getHuskHomes().getManager().warps().setWarpName(warp, newName);
                                                        }
                                                    } catch (ValidationException e) {
                                                        if (position instanceof Home home) {
                                                            final boolean editingOtherHome = !player.getUniqueId().equals(home.getOwner().getUuid());
                                                            e.dispatchHomeError(api.adaptUser(player), editingOtherHome, plugin.getHuskHomes(), newName);
                                                        } else {
                                                            e.dispatchWarpError(api.adaptUser(player), plugin.getHuskHomes(), newName);
                                                        }
                                                        return;
                                                    }
                                                    position.getMeta().setName(newName);
